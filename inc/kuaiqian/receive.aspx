...
//设置人民币网关密钥
///区分大小写
String key = my_b.get_pay("快钱", "zhifukeyma");
...
//那么，GetMD5()方法中所传递的编码方式也必须与此保持一致。
    ...
    my_c.genxin("insert into sl_system (u1,u2,u3,u4) values('fzw','" + orderId + "|" + payResult + "|" + errCode + "|" + payAmount + "','" + Request.UserHostAddress.ToString() + "','支付返回')");
    ...
    if (signMsg.ToUpper() == merchantSignMsg.ToUpper())
    {
        ...
        if (payResult == "10")
        {
            ...
            */
            ...
            my_c.genxin("update sl_order set zhuangtai='已付款' where dingdanhao='" + orderId + "'");

            my_c.genxin("insert into sl_system (u1,u2,u3,u4) values('orderId','" + orderId + "','" + Request.UserHostAddress.ToString() + "','会员订单')");
          
            //报告给快钱处理结果，并提供将要重定向的地址。
            Response.Write("<result>1</result>" + "<redirecturl>" + my_b.get_Domain()  +"inc/kuaiqian/show.aspx?msg=success</redirecturl>");
        }
        else
        {
            Response.Write("<result>0</result>" + "<redirecturl>" + my_b.get_Domain()  +"inc/kuaiqian/show.aspx?msg=success</redirecturl>");
        }
