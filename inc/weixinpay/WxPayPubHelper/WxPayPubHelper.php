...
class Common_util_pub
{
	...
	{		
		...
		logger2("Common_util_pub postXmlCurl-xml:".$xml);
		logger2("Common_util_pub postXmlCurl-url:".$url);
		//初始化curl		
	   	$ch = curl_init();
		//设置超时
		...
		//运行curl
		$data = curl_exec($ch);
		logger2("Common_util_pub postXmlCurl-data:".$data);
		//curl_close($ch);
		//返回结果
		if($data)
		{
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
		...
		$abc =$this->arrayToXml($this->parameters);;
		logger2("Wxpay_client_pub createXml-abc:".$abc);
		return  $abc;
	}
	...
	{
		$xml = $this->createXml();
		logger2("Wxpay_client_pub postXml-xml:".$xml);
...
{	
	...
	function createXml()
	{
		try
		{
			...
			$xyz =$this->arrayToXml($this->parameters);
			logger2("UnifiedOrder_pub createXml-xyz:".$xyz);
			return  $xyz;
