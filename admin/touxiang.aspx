<%@ Page Language="C#" AutoEventWireup="true" CodeFile="touxiang.aspx.cs" Inherits="admin_shangchuan" %>
...
function challs_flash_onCompleteData(a){ //获取服务器反馈信息事件
	if(document.getElementById('show1').innerHTML!="")
	{
...
function challs_flash_onCompleteAll(a){ //上传文件列表全部上传完毕事件,参数 a 数值类型，返回上传失败的数量
	window.opener.document.all('<%=editname %>').value=document.getElementById('show1').innerHTML;
    window.opener.document.all('<%=pic %>').src=document.getElementById('show1').innerHTML;
	alert('上传成功');
	window.close();
}
...
<div id="show1" style="margin-top:20px; width:500px; text-align:left"></div>
</body>
